...
function unlock(frag){
  ...
  if(unlocked.size === 3){
    setTimeout(()=>{
      ...
      setTimeout(()=> synth.classList.add('on'), 1200);
    }, 1400);
  }
}
...
document.getElementById('seal').addEventListener('click', e=>{
  ...
  synth.style.transition = 'opacity 1.2s ease';
  synth.innerHTML = '<div style="font-family:Fraunces,serif;font-style:italic;font-weight:300;font-size:32px;color:#888;text-align:center;text-wrap:pretty;max-width:620px;">The archive is sealed.<br>The room keeps no name.<br>The quiet remains for whoever arrives next.</div>';
});
